...
def main():
    ...
    message = "Rate: {rate} UserID: {userID}"  # template for message for communication with server
    ...
    if int(user_id) == 0:
        ...
        message_sent = message.format(rate=option, userID=user_id)  # send rating and user ID
...
def load_socket():
    ...
    address = r'tcp://localhost:' + str(server_id)
